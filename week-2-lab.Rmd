...
<!-- -->
```
(i) the size of crab and latitude: 0.5
(ii) water temperature and latitude: -0.9
...
print(pie_crab |> summarize(crab_lat = cor(size, latitude))) # 0.59

print(pie_crab |> summarize(temp_lat = cor(water_temp, latitude))) #-0.957
...
```{r}
lm(size ~ latitude, data = pie_crab) %>% tidy %>% gt()
# old pipe operator does not work for this :)

```

```{r}
summary(lm(size ~ latitude, data = pie_crab))
```
...
summary(lm(water_temp ~ latitude, data = pie_crab))
...
ggplot(data = pie_crab, aes(y = water_temp, x = latitude)) +
  geom_point() + 
  geom_smooth(method = "lm", formula= y~x, se=FALSE) +  # set standard errors to false
  labs(x = "Latitude",
       y = "Water temperature (Celsius)") + 
  theme_bw()
...
predictions <- pie_crab %>% add_predictions(model_1) %>% # adds a column "pred" (y-hats)
...
```{r}
# regression
model_2 <- lm(size ~ water_temp, data = pie_crab)

# create predictions and residuals
predictions_2 <- pie_crab %>% add_predictions(model_2) %>% # adds a column "pred" (y-hats)
  mutate(residuals = size-pred)
...
- unimodal
- pretty symmetric
- doesn't really have tails
- "pretty good"
- next step:  Q-Q plot (compare our normal vs theoretical)
...
ggplot(predictions) + 
  ...
  theme_bw()
```
- looks pretty good, might want to do some additional checking but it looks okay
- don't want to see cloud of variance points at one end of the plot
- don't want heteroskedasticity 
